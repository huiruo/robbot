...
export function SettingsModal(props: { open: boolean; variant?: 'modal' | 'page'; email: string; deepseek: string | null; openai: string | null; selectedAi: string | null; onClose(): void; onSave(field: AiField, value: Record<string, unknown>): Promise<void>; onSelect(field: AiField): Promise<void>; onLogout(): void }) {
  ...
  const [selectedModels, setSelectedModels] = useState<Record<AiField, string>>(() => ({ deepseek: readStringValue(props.deepseek, 'model') || 'deepseek-v4-pro', openai: readStringValue(props.openai, 'model') || 'gpt-5.6-luna' }))
  const [saving, setSaving] = useState<AiField | null>(null)
  const [selecting, setSelecting] = useState<AiField | null>(null)
  const fields: Array<{ field: AiField; label: string }> = [{ field: 'deepseek', label: 'DeepSeek' }, { field: 'openai', label: 'ChatGPT' }]
